// ============================================
// SCROLL ANIMATIONS & HEADER EFFECTS
// ============================================
...
// Re-init animations after content changes
...
function loadProductsFromStorage() {
    try {
        ...
        // ✅ MERGE: INITIAL_PRODUCTS bilan localStorage ni yangilash
        ...
        if (typeof INITIAL_PRODUCTS !== 'undefined') {
            INITIAL_PRODUCTS.forEach(initProduct => {
                ...
                const storedProd = storedMap.get(id);

                if (!storedProd) {
                    // Yangi mahsulot - INITIAL_PRODUCTS dan qo'shish
                    storedMap.set(id, initProduct);
                    changed = true;
                } else {
                    // Mavjud mahsulot - maydonlarni tekshirish va yangilash

                    // minQty tekshirish
                    const initMinQty = parseInt(initProduct.minQty) || 1;
                    const storedMinQty = parseInt(storedProd.minQty) || 1;
                    if (storedMinQty !== initMinQty) {
                        storedProd.minQty = initMinQty;
                        changed = true;
                    }

                    // ✅ BARREL QTY - eng muhim qism
                    const initBarrelQty = initProduct.barrelQty || null;
                    const storedBarrelQty = storedProd.barrelQty || null;

                    // Agar stored da barrelQty yo'q bo'lsa, INITIAL dan olish
                    if (!storedBarrelQty && initBarrelQty) {
                        storedProd.barrelQty = initBarrelQty;
                        changed = true;
                    }

                    // Agar admin yangi barrelQty kiritgan bo'lsa (stored da bor), uni saqlash
                    // Bu yerda stored ustunlik beradi, chunki admin yangilagan bo'lishi mumkin

                    // description tekshirish
                    if (storedProd.descriptionRu !== initProduct.descriptionRu) {
                        storedProd.descriptionRu = initProduct.descriptionRu;
                        changed = true;
                    }
                    if (storedProd.descriptionUz !== initProduct.descriptionUz) {
                        storedProd.descriptionUz = initProduct.descriptionUz;
                        changed = true;
                    }
                    if (storedProd.descriptionEn !== initProduct.descriptionEn) {
                        storedProd.descriptionEn = initProduct.descriptionEn;
                        changed = true;
                    }

                    // narxlar tekshirish
                    if (initProduct.prices) {
                        if (!storedProd.prices) storedProd.prices = {};
                        if (initProduct.prices.retail && (!storedProd.prices.retail || storedProd.prices.retail === 0)) {
                            storedProd.prices.retail = initProduct.prices.retail;
                            changed = true;
                        }
                        if (initProduct.prices.wholesale && (!storedProd.prices.wholesale || storedProd.prices.wholesale === 0)) {
                            storedProd.prices.wholesale = initProduct.prices.wholesale;
                            changed = true;
                        }
                    }
                }
        ...
        // ✅ Har bir mahsulot uchun barrelQty ni tekshirish va to'g'rilash
        products.forEach(product => {
            // Agar barrelQty bo'lmasa, descriptiondan parse qilish (fallback)
            if (!product.barrelQty || product.barrelQty <= 0) {
                const descBarrel = parseBarrelFromDescription(product);
                if (descBarrel) {
                    product.barrelQty = descBarrel;
                    changed = true;
                }
            }
        });
        ...
        if (changed) {
            localStorage.setItem('pharmegic_products', JSON.stringify(products));
            console.log('✅ LocalStorage mahsulotlari yangilandi. Jami:', products.length);
        } else {
...
// ✅ YORDAMCHI FUNKSIYA: Descriptiondan barrel miqdorini parse qilish
function parseBarrelFromDescription(product) {
    const descriptions = [
    ...
    for (const desc of descriptions) {
        ...
        // "Bochka: 25 kg" yoki "Бочка: 25 кг" formatini qidirish
        const match = desc.match(/(?:Бочка|Мешок|Банка|Bochka|Drum|Bag|Jar|Бидон|Канистр|бочка|мешок|банка|bochka|drum|bag|jar|bidon|kanistr):\s*(\d+)\s*(?:кг|kg|кг\b|kg\b)/i);
...
// Parse barrel amount from description
function getBarrelAmount(product, lang) {
    // ✅ AVVAL alohida maydonni tekshirish
    if (product.barrelQty && product.barrelQty > 0) {
        return parseInt(product.barrelQty);
    }
    
    // Fallback: descriptiondan parse qilish
    const desc = getProductDescription(product, lang);
    const match = desc.match(/(?:Бочка|Мешок|Банка|Bochka|Drum|Bag|Jar):\s*(\d+)\s*(?:кг|kg)\b/i);
...
function renderProducts(filter = 'all', searchTerm = '') {
   ...
   grid.innerHTML = filteredProducts.map(product => {
       ...
       const barrelAmount = getBarrelAmount(product, currentLang);
       ...
       <div class="product-card" onclick="openProductModal(${product.id})">
           ...
           <div class="product-info">
               ...
               ${barrelAmount ? `<div class="product-barrel-info"><i class="fas fa-drum"></i> Bochka: ${barrelAmount} kg</div>` : ''}
...
async function loadProductsFromAPI() {
    try {
        ...
        // INITIAL_PRODUCTS dan minQty va description ni olish uchun map
        const initialMap = typeof INITIAL_PRODUCTS !== 'undefined'
            ? new Map(INITIAL_PRODUCTS.map(p => [parseInt(p.id), p]))
            : new Map();

        serverProducts.forEach(p => {
            const pid = parseInt(p.id);
            const initProduct = initialMap.get(pid);
            const localProduct = localMap.get(pid);

            // ✅ minQty: INITIAL_PRODUCTS dan ustunlik berish (server noto'g'ri bo'lishi mumkin)
            const minQty = initProduct ? parseInt(initProduct.minQty) || 1 : (p.min_qty || 1);

            // ✅ barrelQty: avval local/initial, keyin server
            let barrelQty = null;
            
            // 1. LocalStorage dan (admin kiritgan)
            if (localProduct && localProduct.barrelQty && localProduct.barrelQty > 0) {
                barrelQty = parseInt(localProduct.barrelQty);
            }
            // 2. INITIAL_PRODUCTS dan
            else if (initProduct && initProduct.barrelQty && initProduct.barrelQty > 0) {
                barrelQty = parseInt(initProduct.barrelQty);
            }
            // 3. Server dan (barrel_qty yoki barrelQty)
            else if (p.barrel_qty || p.barrelQty) {
                barrelQty = parseInt(p.barrel_qty || p.barrelQty) || null;
            }
            // 4. Description dan parse qilish (fallback)
            else {
                const desc = p.description_uz || p.description_ru || p.description_en || '';
                const match = desc.match(/(?:Бочка|Мешок|Банка|Bochka|Drum|Bag|Jar):\s*(\d+)\s*(?:кг|kg)\b/i);
                if (match) barrelQty = parseInt(match[1]);
            }

            // ✅ description: INITIAL_PRODUCTS dan ustunlik berish
            const descRu = initProduct ? initProduct.descriptionRu : (p.description_ru || '');
            const descUz = initProduct ? initProduct.descriptionUz : (p.description_uz || '');
            const descEn = initProduct ? initProduct.descriptionEn : (p.description_en || '');

            // ✅ narxlar: server dan, lekin 0 bo'lsa initial dan
            let prices = typeof p.prices === 'object' ? p.prices : (typeof p.prices === 'string' ? JSON.parse(p.prices) : {retail: 0, wholesale: 0});
            
            if (initProduct && initProduct.prices) {
                if (!prices.retail || prices.retail === 0) prices.retail = initProduct.prices.retail;
                if (!prices.wholesale || prices.wholesale === 0) prices.wholesale = initProduct.prices.wholesale;
            }
...
function addToCart(productId, quantity) {
   ...
   // ✅ ENDI: 1 kg dan kam bo'lsa ham ruxsat (faqat 0 dan katta bo'lishi kerak)
   const qty = typeof quantity !== 'undefined' ? Math.max(parseInt(quantity) || 1, 1) : minQty;
...
function updateQuantity(index, change) {
   ...
   if (item) {
       ...
       const product = products.find(p => p.id === item.id);
       const barrelAmount = product ? getBarrelAmount(product, currentLang) : null;
       const step = barrelAmount && currentQty >= barrelAmount ? barrelAmount : minQty;

       item.quantity = currentQty + (change * step);

       if (item.quantity < minQty) {
           item.quantity = minQty; // Don't go below minQty
       }
...
function openProductModal(productId) {
   ...
   // ✅ UNIQUE preset qiymatlar (takrorlanmasin)
   const minQtyVal = parseInt(product.minQty) || 1;
   const barrelAmount = getBarrelAmount(product, currentLang);
   const presetValues = [minQtyVal];
   ...
   modalBody.innerHTML = `
       <div class="product-modal-layout">
           ...
           <div class="product-modal-info">
               ...
               ${barrelAmount ? `<div class="min-qty-info modal barrel"><i class="fas fa-drum"></i> ${currentLang === 'uz' ? 'Bochka' : currentLang === 'ru' ? 'Бочка' : 'Drum'}: ${barrelAmount} kg</div>` : ''}

               <div class="quantity-selector">
                   <label>${t.quantity}:</label>
                   <div class="quantity-input-group">
                       <button onclick="adjustModalQuantity(-1, ${product.id})">-</button>
                       <input type="number" id="modalQuantity" value="${minQtyVal}" min="${minQtyVal}" step="${minQtyVal}" onchange="validateQuantity(this, ${minQtyVal})">
                       <button onclick="adjustModalQuantity(1, ${product.id})">+</button>
                   </div>
               </div>

               <div class="preset-qty-buttons">
                   ${presetValues.map(val => `
                       <button type="button" class="preset-btn" onclick="setModalQuantity(${val})">${val} kg</button>
                   `).join('')}
                   ${barrelAmount ? `<button type="button" class="preset-btn barrel-btn" onclick="setModalQuantity(${barrelAmount})">${currentLang === 'uz' ? 'Bochkaga' : currentLang === 'ru' ? 'Бочка' : 'Drum'}</button>` : ''}
               </div>
...
function adjustModalQuantity(direction, productId) {
   const input = document.getElementById('modalQuantity');
   if (!input) return;

   const product = products.find(p => String(p.id) === String(productId));
   const minQty = product ? Math.max(parseInt(product.minQty) || 1, 1) : 1;
   const barrelAmount = product ? getBarrelAmount(product, currentLang) : null;
   const currentQty = Math.max(parseInt(input.value) || minQty, minQty);

   let step = minQty;
   if (barrelAmount && currentQty >= barrelAmount) {
       step = barrelAmount;
   }

   let value = currentQty + direction * step;
...
async function submitIndividualOrder(event) {
    ...
    const orderData = {
        ...
        items: cart.map(item => {
            ...
            const barrelAmount = product ? (getBarrelAmount(product, currentLang) || 25) : 25;
            const itemTotal = item.quantity >= barrelAmount ? item.wholesalePrice * item.quantity : item.price * item.quantity;
            const price = item.quantity >= barrelAmount ? item.wholesalePrice : item.price;
            return {
    ...
    const orderData = {
        ...
        items: cart.map(item => {
            ...
            return {
                name: getProductName(item, currentLang),
                quantity: item.quantity,
                price: item.quantity >= barrelAmount ? item.wholesalePrice : item.price
            };
        }),
        total: total,
...
function setLang(lang) {
    ...
    // ✅ Tanlangan til tugmasiga active klassini qo'shish
    document.querySelectorAll('.lang-btn').forEach(btn => {
...
if (savedLang) {
    // Avval barcha active klasslarini olib tashlash
    document.querySelectorAll('.lang-btn').forEach(btn => btn.classList.remove('active'));
    setLang(savedLang);
} else {
    // Default UZ - active klass allaqachon HTML da bor
    setLang('uz');
}